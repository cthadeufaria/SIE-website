...
<body>
	<?php
	  ...
	  if (!empty($_GET['cancel']))												// If the user clicked the cancel button
		 	  Header("Location: listFriends.php");								// redirect to the home (list) page
	  else {																	// Otherwise (user clicked confirm) update friend as in the previou version
																				// After the update, redirect to the home page.
				// **********  STEP 1
				$id   		 = $_POST['friendId'];
				...
				if (!empty($_FILES["photo"]["name"]))  {			//If the user uploaded a photo in the form, then we should:
					...
					$photo  = $row['photo'];
					if (!empty($photo))
						unlink('../../photos/' . $photo);					//Note: unlink() is the PHP command to delete a file in the file system
